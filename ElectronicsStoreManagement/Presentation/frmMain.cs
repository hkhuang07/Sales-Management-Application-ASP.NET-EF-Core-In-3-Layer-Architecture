           ...
           OpenFileDialog restoreDialog = new OpenFileDialog();
            ...
            if (restoreDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!string.IsNullOrEmpty(filePath))
                {
                    ...
                    {
                        MessageBox.Show("Data recovery failed!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }
...
>>>>>>> 3065499e331bff93273813c88f9001fae0918d8c
        ...
        private void Member()
        {
            ...
            mnuData.Enabled = false;
            mnuSale.Enabled = true;
            mnuCategories.Enabled = false; // Nhân viên không có quyền truy cập vào danh sách danh mục
            mnuManufacturers.Enabled = false; // Nhân viên không có quyền truy cập vào danh sách nhà sản xuất
            mnuProducts.Enabled = true;
            mnuCustomers.Enabled = true;
            mnuEmployees.Enabled = false; // Nhân viên không có quyền truy cập vào danh sách nhân viên
            reportStatisticToolStripMenuItem.Enabled = false;// Nhân viên không có quyền truy cập vào thống kê
            tabLogin.Enabled = false;
            tabLogout.Enabled = true;
            tabChangepass.Enabled = true;
            tabRestore.Enabled = true;   
